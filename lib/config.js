...
module.exports = class Config {
  ...
  validateOpts (opts) {
    ...
    res.ignore.unshift(
      ...
      'spike.config.js', // primary config
      'spike.config.*.js' // any environment config
    )
  ...
  parseAppJs (opts) {
    ...
    let config = loadFile(path.resolve(opts.root, 'spike.config.js'))

    if (opts.env) {
      const envConfig = loadFile(path.resolve(opts.root, `spike.config.${opts.env}.js`))
